...
context1.load_verify_locations('CA.pem')

context3 = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
context3.load_verify_locations('CA.pem')
...
def caller(con):
    with lock:
        con.sendall(str(shares.pop()).encode('utf-8'))
...
def handleserver(con):
    ...
    contextOwnServer.load_cert_chain(certfile='client2.crt', keyfile='client2.key')
    contextOwnServer.load_verify_locations('CA.pem')
...
con3_thread = threading.Thread(target=connector, args=(port_3, context3))
con2_thread.start()
con3_thread.start()
